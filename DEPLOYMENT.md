...
npm run type-check
yarn workspace @digital-astrology/web lint
...
yarn workspace @digital-astrology/web lint --fix
...
ls -la apps/web/.next/server/app/privacy
ls -la apps/web/.next/server/app/terms
ls -la apps/web/.next/server/app/refund-policy
...
2. Verify CookieBanner component in layout:
   ```bash
   grep -n "CookieBanner" apps/web/app/layout.tsx
   ```
...
3. Verify booking modal component:
   ```bash
   grep -n "disclaimerAccepted" apps/web/components/consultation/booking-modal.tsx
   ```
