...
describe("useSavedCharts - Favorite Toggle Loading State", () => {
  ...
  it("should allow toggling different charts simultaneously", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining("chart-1"), expect.any(Object));
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining("chart-2"), expect.any(Object));
  });
