 ...
 */
export function delayedPromise<T>(value: T, delayMs = 100): Promise<T> {
 ...
 */
export function rejectedPromise(error: Error | string, delayMs = 0): Promise<never> {
  return new Promise((_, reject) => {
    setTimeout(() => reject(typeof error === "string" ? new Error(error) : error), delayMs);
  });
}
