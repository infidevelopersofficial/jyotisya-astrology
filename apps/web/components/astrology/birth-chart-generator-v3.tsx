...
export default function BirthChartGeneratorV3(): React.ReactElement {
  ...
  const handleDownloadPNG = async () => {
    ...
    try {
      ...
      const fullChartName = selectedDivisional !== 'D1' ? `${displayName} – ${divisionalChartName}` : displayName
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-center gap-2 md:gap-4">
        <div className={`flex items-center gap-2 ${activeTab === 'form' ? 'text-orange-400' : chartData ? 'text-green-400' : 'text-slate-500'}`}>
          <div className={`flex h-10 w-10 items-center justify-center rounded-full border-2 font-bold ${activeTab === 'form' ? 'border-orange-400 bg-orange-400/20' : chartData ? 'border-green-400 bg-green-400/20' : 'border-slate-500 bg-slate-500/10'}`}>
            {chartData ? '✓' : '1'}
          </div>
        ...
        <div className={`flex items-center gap-2 ${activeTab === 'chart' ? 'text-orange-400' : chartData ? 'text-green-400' : 'text-slate-500'}`}>
          <div className={`flex h-10 w-10 items-center justify-center rounded-full border-2 font-bold ${activeTab === 'chart' && chartData ? 'border-orange-400 bg-orange-400/20' : chartData ? 'border-green-400 bg-green-400/20' : 'border-slate-500 bg-slate-500/10'}`}>
            {chartData && activeTab !== 'chart' ? '✓' : '2'}
          </div>
          <span className="hidden text-sm font-semibold md:inline">View Chart</span>
        </div>

        <div className={`h-0.5 w-12 md:w-24 ${activeTab === 'divisional' && chartData ? 'bg-green-400' : 'bg-slate-600'}`} />

        <div className={`flex items-center gap-2 ${activeTab === 'divisional' && chartData ? 'text-orange-400' : 'text-slate-500'}`}>
          <div className={`flex h-10 w-10 items-center justify-center rounded-full border-2 font-bold ${activeTab === 'divisional' && chartData ? 'border-orange-400 bg-orange-400/20' : 'border-slate-500 bg-slate-500/10'}`}>
            3
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-purple-500/30 bg-purple-500/10 p-4">
                ...
                <p className="text-sm text-purple-300">
                  Your ascendant changes every ~2 hours. Even 4-5 minutes can shift house placements. Check your birth certificate for accuracy.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-orange-500/30 bg-orange-500/10 p-4">
                ...
                <p className="text-sm text-orange-300">
                  Your birth chart is a cosmic snapshot of the sky at your birth moment. It reveals personality, strengths, and life path.
                </p>
              </div>
            </div>
          )}
